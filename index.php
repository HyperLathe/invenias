<!DOCTYPE HTML>

<html>

<?php $pageTitle = "Invenias | Home";
       $metaDescription = "Invenias is the world’s fastest growing provider of software solutions to the executive search and strategic recruitment sector.";
	   $metaKeywords = "Invenias, executive search software, recruitment, software, recruitment software, invenias executive search software inside Microsoft outlook, Microsoft outlook, executive search";
	   include($DOCUMENT_ROOT . "./includes/head.php");
?>


<body>

  <?php include($DOCUMENT_ROOT . "./includes/header.php"); ?>
  
  
  <!-- Start of Carousel -->
  <div id="carousel-example-generic" class="carousel slide" data-ride="carousel">
  <!-- Indicators -->
  <div class="container-fluid">
    <ol class="carousel-indicators hidden-xs">
      <li data-target="#carousel-example-generic" data-slide-to="0" class="active"></li>
      <li data-target="#carousel-example-generic" data-slide-to="1"></li>
      <li data-target="#carousel-example-generic" data-slide-to="2"></li>
    </ol>
  </div>

  <!-- Wrapper for slides -->
  <div class="carousel-inner">
    <div class="item active">
      <img src="images/slide_01.jpg" alt="Powerful and easy to use executive search software delivered within Outlook" class="desktop_hero_slide">
      <img src="images/slide_01_tablet.jpg" alt="Powerful and easy to use executive search software delivered within Outlook" class="tablet_hero_slide">
      <img src="images/slide_01_mobile.jpg" alt="Powerful and easy to use executive search software delivered within Outlook" class="mobile_hero_slide">
      <div class="container-fluid">
        <div class="carousel-caption hidden-xs">Powerful and easy to use executive search software delivered inside Outlook</div>
      </div>
    </div>
    <div class="item">
      <img src="images/slide_02.jpg" alt="Powerful and easy to use executive search software delivered within Outlook" class="desktop_hero_slide">
      <img src="images/slide_02_tablet.jpg" alt="Powerful and easy to use executive search software delivered within Outlook" class="tablet_hero_slide">
      <img src="images/slide_02_mobile.jpg" alt="Powerful and easy to use executive search software delivered within Outlook" class="mobile_hero_slide">
      <div class="container-fluid">
        <div class="carousel-caption hidden-xs">Invenias...Discover... Better delivery<br>Better relationships<br>Better results</div>
      </div>
    </div>
    <div class="item">
      <img src="images/slide_03.jpg" alt="Powerful and easy to use executive search software delivered within Outlook" class="desktop_hero_slide">
      <img src="images/slide_03_tablet.jpg" alt="Powerful and easy to use executive search software delivered within Outlook" class="tablet_hero_slide">
      <img src="images/slide_03_mobile.jpg" alt="Powerful and easy to use executive search software delivered within Outlook" class="mobile_hero_slide">
      <div class="container-fluid">
        <div class="carousel-caption hidden-xs">Managed in the Cloud, delivered on your desktop, available on your mobile</div>
      </div>
    </div>
  </div>

  <!-- Controls -->
  <a class="left carousel-control" href="#carousel-example-generic" role="button" data-slide="prev"></a>
  <a class="right carousel-control" href="#carousel-example-generic" role="button" data-slide="next"></a>
</div>
<!-- End of Carousel -->


<!-- Start of main content -->
<div class="home_content">
<div class="container-fluid">
    <div class="narrow_centred_copy">
      <!--<p class="intro_para">Invenias is the world’s fastest growing provider of software solutions to the executive search and strategic recruitment sector.</p>
      <p class="hidden-xs">We enable our customers to fulfil search assignments more effectively, build stronger relationships with their clients and candidates, and transform the productivity of their operations. Customers love our easy to use desktop and mobile apps, and our Cloud platform which removes complexity and cost.</p>-->
      <p class="intro_para">Invenias is the world’s fastest growing provider of software solutions to the executive search and specialist recruitment sector.</p>
      <p class="hidden-xs">Single users, boutique firms, multi-national organizations and corporate executive recruiting teams all use Invenias. We enable our customers to fulfil search assignments more effectively, build stronger relationships with their clients and candidates, and transform the productivity of their operations. Thousands of users world-wide love our easy to use desktop and mobile apps, and our Cloud-based platform which removes risk, complexity and cost.</p>
    </div>
    <div class="row">
      <div class="col-xs-12 col-sm-3 col-md-3">
        <div class="thumbnail">
          <a href="#"><img src="images/home_feature1.jpg" class="img-responsive" alt="View Online Demo">
          <div class="caption"><div class="btn btn-primary" role="button">VIEW ONLINE DEMO</div></div></a>
        </div>
      </div>
      <div class="col-xs-12 col-sm-3 col-md-3">
        <div class="thumbnail">
          <a href="customers.php"><img src="images/home_feature2.gif" class="img-responsive" alt="Global Locations">
          <div class="caption"><div class="btn btn-primary" role="button">GLOBAL LOCATIONS</div></div></a>
        </div>
      </div>
      <div class="col-xs-12 col-sm-3 col-md-3">
        <div class="thumbnail">
          <a href="#"><img src="images/home_feature3.jpg" class="img-responsive" alt="Try it for Free">
          <div class="caption"><div class="btn btn-primary" role="button">TRY IT FOR FREE</div></div></a>
        </div>
      </div>
      <div class="col-xs-12 col-sm-3 col-md-3">
        <div class="thumbnail">
          <a href="customers.php"><!--<img alt="View Online Demo" class="img-responsive" src="images/home_feature4.jpg">-->
          <div class="cycle-slideshow" data-cycle-speed="500" data-cycle-timeout="2000">
            <img src="images/slides/slide_normanbroadbent.gif" class="img-responsive" alt="Norman Broadbent">
            <img src="images/slides/slide_sheffieldhaworth.gif" class="img-responsive" alt="Sheffield Haworth">
            <img src="images/slides/slide_mulsanne.gif" class="img-responsive" alt="Mulsanne">
            <img src="images/slides/slide_allenaustin.gif" class="img-responsive" alt="Allen Austin">
            <img src="images/slides/slide_daubenspeck.gif" class="img-responsive" alt="Daubenspeck">
            <img src="images/slides/slide_ds.gif" class="img-responsive" alt="DS">
            <img src="images/slides/slide_gtsearch.gif" class="img-responsive" alt="GT Search">
            <img src="images/slides/slide_healyhunt.gif" class="img-responsive" alt="Healy Hunt">
            <img src="images/slides/slide_highfield.gif" class="img-responsive" alt="Highfield">
            <img src="images/slides/slide_rpi.gif" class="img-responsive" alt="RPI">
            <img src="images/slides/slide_talenses.gif" class="img-responsive" alt="Talenses">
            <img src="images/slides/slide_mccracken.gif" class="img-responsive" alt="McCracken">
            <img src="images/slides/slide_sportsrecruitment.gif" class="img-responsive" alt="Sports Recruitment International">
            <img src="images/slides/slide_emerald.gif" class="img-responsive" alt="Emerald">
          </div>
          <div class="caption"><div class="btn btn-primary" role="button">CUSTOMER CASE STUDIES</div></div></a>
        </div>
      </div>
    </div>
  </div>
</div>

<div class="home_content_grey">
  <div class="container-fluid">
    <div class="narrow_centred_copy">
      <p class="intro_para">LATEST NEWS &AMP; HIGHLIGHTS</p>
    </div>
    <div class="row">
      <div class="col-xs-12 col-sm-4 col-md-4">
        <div class="thumbnail_variant">
            <a href="#"><img src="images/news_mobileproviders.gif" class="img-responsive" alt="Mobile Providers">
            <h4>Invenias Available on All Four Major Mobile Platforms</h4>
            <p>September 2, 2014<br>Read More...</p>
            </a>
        </div>
      </div>
      <div class="col-xs-12 col-sm-4 col-md-4">
         <div class="thumbnail_variant">
            <a href="#"><img src="images/news_microsoftgold.jpg" class="img-responsive" alt="Microsoft Gold">
            <h4>Invenias becomes a Microsoft Gold Partner</h4>
            <p>September 2, 2014<br>Read More...</p>
            </a>
          </div>
        </div>
        <div class="col-xs-12 col-sm-4 col-md-4">
          <div class="thumbnail_variant">
            <a href="#"><img src="images/news_chloeturner.gif" class="img-responsive" alt="Chloe Turner">
            <h4>Chloe Turner, Head of Insight, The Up Group</h4>
            <p>September 2, 2014<br>Read More...</p>
            </a>
          </div>
        </div>
      </div>
      <hr>
      <div class="narrow_centred_copy">
        <p class="intro_para">WE'RE HIRING</p>
        <p><a href="#">Click here to see current opportunities ></a></p>
      </div>
  </div>
</div>


<!-- End of main content -->

  
  
    <?php include($DOCUMENT_ROOT . "./includes/footer.php"); ?>
    
</body>
</html>